...
from vtk_viewer import GSIPing
from vtk_viewer import gc_constants as gc
from PyQt5.QtCore import pyqtSignal, pyqtSlot, QTimer, QThread
from PyQt5.QtWidgets import QInputDialog, QWidget
from qgis.core import QgsMessageLog
from vtk_viewer import GeoCOMRequest, GeoCOMMessageQueue, GeoCOMReply, GeoCOMPing
from vtk_viewer import gc_constants
...
class TachyReader(QThread):
    ...
    def hook_up(self):
        ...
        for port in QSerialPortInfo.availablePorts():
            if "Prolific" in port.manufacturer():
                ...
                return
        print(
            f"'Prolific' not found in Port list: {[port.manufacturer() for port in QSerialPortInfo.availablePorts()]}"
        )
        # Show port list and let user choose the port
        btPort, okPressed = QInputDialog.getItem(
            QWidget(),
            "Verbinden...",
    ...
    def setReflectorHeight(self, refHeight):
        ...
        if self.ser.isOpen():
            ...
            self.ser.writeData(
                ("PUT/87...0+" + gsi_height[0].zfill(5) + "{:<03s}".format(gsi_height[1]) + " \r\n").encode("ascii")
            )
