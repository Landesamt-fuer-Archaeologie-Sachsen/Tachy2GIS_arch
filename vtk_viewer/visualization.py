...
class VtkLayer:
    ...
    def add_feature(self, vertices):
        ...
        if isinstance(wktGeo, list):
            ...
            for geo in wktGeo:
                features.append(
                    QgsVectorLayerUtils.createFeature(
                        self.source_layer, QgsGeometry.fromWkt(geo), {}, self.source_layer.createExpressionContext()
                    )
                )
        else:
            ...
            features = [
                QgsVectorLayerUtils.createFeature(
                    self.source_layer, geometry, {}, self.source_layer.createExpressionContext()
                )
            ]
